...
const RunTutorial = ({children}: { children: ReactNode }) => {
    const [runStatus, setRunStatus] = useState<"not started" | "running" | "waiting sidebar" | "finished">("running")
    ...
    const [steps, setSteps] = useState<Step[]>([
        {
            target: "body",
            content: <TourContent>
                Esta es la wiki de Cabildo Abierto
            </TourContent>,
            placement: 'center',
            disableBeacon: true,
            hideBackButton: true
        ...
        },
        {
            target: '#topic-search-result',
            content: <TourContent>
                Cada tema tiene una página propia, podés entrar haciendo clic cuando termine el tutorial. Las páginas de temas tienen un artículo que cualquiera puede editar y la discusión sobre el tema.
            </TourContent>,
            placement: 'bottom',
            disableBeacon: true,
            hideBackButton: true
