...
export const TableVisualizationConfig = ({config, setConfig}: TableVisualizationConfigProps) => {
    const {data, isLoading: loadingTopicsDataset} = useTopicsDataset(isTopicsDataSource(config.dataSource) ? config.filters : null, true)
    const {data: dataset, isLoading: loadingDataset} = useDataset(isDatasetDataSource(config.dataSource) ? config.dataSource.dataset : null)
    ...
    function setShowColumn(c: string, v: boolean, alias?: string){
        setConfig(produce(config, draft => {
            if(isTable(draft.spec)){
                if(!draft.spec.columns) draft.spec.columns = []
                if(v){
    ...
    return <div className={"space-y-1"}>
        ...
        {columns.map((c, i) => {
            ...
            const configIndex = config.spec.columns.findIndex(colConfig => colConfig.columnName == c.name)

            return <div key={i} className={"flex justify-between space-x-2 items-center"}>
                <div className={"text-sm flex-1"}>
                    {c.name}
                </div>
                {configIndex != -1 && <div className={"flex-1"}>
                    <TextField
                        dense={true}
                        label={"Alias"}
                        fontSize={"12px"}
                        size={"small"}
                        value={config.spec.columns[configIndex].alias ?? config.spec.columns[configIndex].columnName}
                ...
                <div>
                    <Switch
                        size={"small"}
                        value={value}
                        onChange={e => {setShowColumn(c.name, e.target.checked)}}
                    />
                </div>
            </div>
        })}
