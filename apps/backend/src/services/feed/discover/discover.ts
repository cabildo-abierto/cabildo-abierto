...
const getDiscoverFeedSkeleton: GetSkeletonProps = (ctx, agent, cursor) => Effect.gen(function* () {
    ...
    const skeleton = yield* Effect.tryPromise({
        try: () => ctx.kysely
            ...
            .select(["DiscoverFeedIndex.contentId", "DiscoverFeedIndex.createdAt as created_at"])
            .where("DiscoverFeedIndex.createdAt", "<", firstFetchDate)
            .where("DiscoverFeedIndex.createdAt", "<", dateSince)
            .where("DiscoverFeedIndex.categoryId", "in", interests.map(i => i.id))
            .orderBy("DiscoverFeedIndex.createdAt", "desc")
            .distinct()
            .limit(25)
            .execute(),
        catch: (error) => new DBSelectError(error)
