...
import {TopicsDataSourceConfig} from "./topics-data-source-config";
...
import {StateButton, StateButtonClickHandler} from "../../utils/base/state-button";
...
const DatasetsSearch = ({datasets, config, setConfig}: {
    ...
    const [filteredDatasets, setFilteredDatasets] = useState<ArCabildoabiertoDataDataset.DatasetViewBasic[] | undefined>(datasets)
...
const ChooseDatasetPanelDatasetSelection = ({
    ...
    return <>
        ...
        <div className={"px-2 pb-2"}>
            {!creatingTopicsBased && datasets && <DatasetsSearch
                datasets={datasets}
                setConfig={setConfig}
                config={config}
            />}
...
export const ChooseDatasetPanelFiltersConfig = ({
    ...
    function onRemoveFilter(i: number) {
        if(setConfig) {
            setConfig(produce(config, draft => {
                if(draft.filters) draft.filters.splice(i, 1)
            }))
        }
    }
    ...
    const dataset = datasets?.find(d => d.uri == datasetUri)
