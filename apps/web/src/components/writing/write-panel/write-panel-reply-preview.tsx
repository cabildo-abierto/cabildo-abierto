...
export const WritePanelReplyPreview = ({replyTo, selection}: Props) => {
    if (selection) {
        if (ArCabildoabiertoWikiTopic.isTopicView(replyTo) || ArCabildoabiertoFeedDefs.isFullArticleView(replyTo)) {
            return <div className={"pb-2 pr-2"}>
                <SelectionQuote
                    quotedContent={replyTo.uri}
                    quotedText={replyTo.text}
                    quotedTextFormat={replyTo.format}
                    quotedContentEmbeds={(ArCabildoabiertoFeedDefs.isFullArticleView(replyTo) || ArCabildoabiertoWikiTopic.isTopicView(replyTo)) ? replyTo.embeds : []}
