...
import {useWorkerState} from "@/queries/getters/admin";
import {Badge} from "@/components/utils/ui/badge";
...
type JobStatus = "active" | "failed" | "waiting" | "delayed" | "completed" | "prioritized"
...
function JobStatusBadge({status}: {status: JobStatus}) {
    const config = {
        ...
        prioritized: {bg: "bg-purple-700", label: "Priorizado"}
    }
...
function JobsTable({jobs, status}: {jobs: WorkerState["activeJobs"], status: JobStatus}) {
    if (jobs.length === 0) {
        return <div className="text-[var(--text-light)] text-sm py-4 text-center">No hay trabajos</div>
    }
                ...
                <tbody>
                    {jobs.map((job, i) => (
                        <tr key={i} className="border-b border-[var(--background-dark2)] hover:bg-[var(--background-dark2)]">
                            ...
                            <td className="py-2 px-3"><JobStatusBadge status={status}/></td>
    ...
    return (
        <div className="flex flex-col items-center mt-8 space-y-8 mb-64 px-4">
            <AdminSection title="Estado del Worker">
                {workerState && <div className={"flex space-x-2 items-center"}>
                    <Badge>
                        {workerState.paused ? "Worker pausado": "Worker corriendo"}
                    </Badge>
                    {workerState.paused && <BaseButton variant={"outlined"} size="small" onClick={async () => {
                        await post<{}, {}>("/resume-worker")
                        await refetch()
                    }}>
                        Resume worker
                    </BaseButton>}
                    {!workerState.paused && <BaseButton variant={"outlined"} size={"small"} onClick={async () => {
                        await post<{}, {}>("/pause-worker")
                        await refetch()
                    }}>
                        Stop worker
                    </BaseButton>}
            ...
            {workerState && (
                <>
                    <AdminSection title="Trabajos activos">
                        <JobsTable jobs={workerState.activeJobs} status={"active"}/>
                    </AdminSection>

                    <AdminSection title="Trabajos delayed">
                        <JobsTable jobs={workerState.delayedJobs} status={"delayed"}/>
                    </AdminSection>

                    <AdminSection title="Trabajos waiting">
                        <JobsTable jobs={workerState.waitingJobs} status={"waiting"}/>
                    </AdminSection>

                    <AdminSection title="Trabajos priorizados">
                        <JobsTable jobs={workerState.prioritizedJobs} status={"prioritized"}/>
                    </AdminSection>
