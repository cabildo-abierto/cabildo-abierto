...
export const CurrentCabildo = ({curCabildoName}: {curCabildoName?: string}) => {
    ...
    if(curCabildoName == undefined) {
        ...
        return <div
            className={"flex flex-col items-center w-full space-y-1 bg-[var(--background-dark)] rounded p-2"}>
            ...
            <div className={"flex justify-between w-full"}>
                ...
                <div className={"text-[var(--text-light)] text-sm flex space-x-1 items-center"}>
                    <div>{curCabildo.cabildo.members.length}</div> <PersonIcon fontSize={"inherit"}/>
                </div>
                <div className={"w-16 text-[var(--text-light)]"}>
                    <BasicButton onClick={() => {setChanging(!changing)}} variant={"text"} color={changing ? "primary" : "inherit"} size={"small"}>
            ...
            {changing && <div className={"flex flex-col space-y-1 w-full"}>
                ...
                {filteredCabildos.length == 0 && <div className={"pt-2 text-sm text-center text-[var(--text-light)]"}>No se encontró el cabildo.</div>}
            </div>}
